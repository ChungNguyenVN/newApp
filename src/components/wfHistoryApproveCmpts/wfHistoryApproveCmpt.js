...
import style from "./wfHistoryApproveCmpt.css";
...
export default class HistoryApprove extends Component {
  constructor(props) {
    super(props);
    ...
    if (this.dataHistory.hasNext) {
      ...
      this.dataHistory["results"].forEach((element) => {
        ...
        items.push({
          ...
          Title: (
            <a
              href={`${config.pages.wfRequestView}?WFTableId=${element.WFTableId}&ItemIndex=${element.ItemIndex}&indexStep=${element.indexStep}&PreviousPage=wfHistoryApprove`}
            >
              {element.Title}
            </a>
          ),
            ...
            <Grid item sm={6} xs={12}>
              ...
              <FormControl fullWidth className="selectForm" variant="outlined">
                ...
                {this.state.ListTypeRequestSearch.length > 0 ? (
                  <div className="tagName">
                    {this.state.ListTypeRequestSearch.map((item) => (
                      <div key={item.ID} className="wrapName">
                        <a
  ...
  async SearchList(sort, sortBy, rowsPerPage) {
    ...
    if (this.currentUser != null) {
      queryFilter += "HistoryApprover/ID eq " + this.currentUser.Id + " ";
    }
